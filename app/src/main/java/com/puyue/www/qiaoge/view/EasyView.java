...
public class EasyView extends ViewGroup{
    private static final String TAG = "slide/SwipeMenuLayout";
    ...
    //存储的是当前正在展开的View
    private static EasyView mViewCache;
...
//    private android.util.Log LogUtils;
     ...
     * @param ios
     */
    public EasyView setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public EasyView setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static EasyView getViewCache() {
        return mViewCache;
    }
...
//                    LogUtils.i(TAG, "dispatchTouchEvent = ACTION_UP、ACTION_CANCEL");
                    //2016 11 03 add,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
                    if (Math.abs(ev.getRawX() - mFirstP.x) > mScaleTouchSlop) {
//                        LogUtils.i(TAG, "手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件");
                        isUserSwiped = true;
                    }
                    //执行关闭
                    boolean opeartionClose = false;
                    //add by 2016 09 11 ，IOS模式开启的话，且当前有侧滑菜单的View，且不是自己的，就该拦截事件咯。滑动也不该出现
                    if (!iosInterceptFlag) {//且滑动了 才判断是否要收起、展开menu
//                        LogUtils.i(TAG, "且滑动了 才判断是否要收起、展开menu");
                        //求伪瞬时速度
                        verTracker.computeCurrentVelocity(1000, mMaxVelocity);
                        final float velocityX = verTracker.getXVelocity(mPointerId);
                        if (Math.abs(velocityX) > 1000) {//滑动速度超过阈值
//                            LogUtils.i(TAG, "滑动速度超过阈值");
                            if (velocityX < -1000) {
//                                LogUtils.i(TAG, "-1000");
                                if (isLeftSwipe) {//左滑
                                    //平滑展开Menu
...
//                            LogUtils.i(TAG, "否则就判断滑动距离 =" + (Math.abs(getScrollX()) > mLimit));
...
//                    LogUtils.i(TAG, "没有手指在摸我了 =" + opeartionClose);

                    if (opeartionClose) {
//                        LogUtils.i(TAG, "因为收菜单，消耗掉此次事件");
                        return true;
                    }
...
//        LogUtils.i(TAG, "super.dispatchTouchEvent(ev)");
...
//                    LogUtils.i(TAG, "onInterceptTouchEvent = ACTION_UP");
                    ...
                    if (isUserSwiped) {
//                        LogUtils.i(TAG, "判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。");
                        return true;
                    }
...
//                LogUtils.i(TAG, "/IOS模式开启，且当前有菜单的View，且不是自己的 拦截点击事件给子View");
                return true;
            }
        }
//        LogUtils.i(TAG, "super.onInterceptTouchEvent(ev);");
...
//    }
    ...
    private boolean isExpand;//代表当前是否是展开状态 2016 11 03 add true
